...
describe User do
  ...
  SAMPLE_DATA = { :ip => "127.0.0.1" }.freeze
  ...
  describe ".with_ip" do
    ...
    shared_examples_for "find the user by ip" do
      it "should contain the user" do
        User.with_ip(sample(:ip)).should include(user)
      end
    end

    context "the user is currently signed in" do
      before do
        user.current_sign_in_ip = sample(:ip)
        user.save!
      end
    ...
    context "user was previously signed in with this ip" do
      before do
        user.last_sign_in_ip = sample(:ip)
        user.save!
      end
    ...
    context "user is not or was prevously not signed in with this ip address" do
      it "should not contain the user" do
        User.with_ip(sample(:ip)).should_not include(user)
      end
    end
  end
end
